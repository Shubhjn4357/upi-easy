      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        {organizations?.length > 0 && (
          <div className="relative">
            ...
            <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2.5 text-muted-foreground text-xs">▼</div>
          </div>
        )}
      </div>
        ...
        {user && (
          <div className="flex items-center gap-2 pl-1">
            <Avatar className="w-7 h-7">
              <AvatarImage
                src={user.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.fullName || 'User')}&background=4F46E5&color=fff`}
                alt={user.fullName}
              />
