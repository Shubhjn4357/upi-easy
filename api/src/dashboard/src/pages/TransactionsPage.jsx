// Transactions & Ledger Explorer Page using shadcn/ui & Action Bottom Sheet
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            size="sm"
            onClick={() => {
              const csv = "ID,Amount,Status,Direction,Payer,VPA,RefNo,Date\n" + transactions.map(t => 
                `"${t.id}",${t.amount},"${t.status}","${t.direction}","${t.payerName || ''}","${t.payeeVpa}","${t.referenceNumber || ''}","${t.occurredAt}"`
              ).join("\n");
      ...
      <Card className="p-3.5 flex flex-col md:flex-row items-center gap-3">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          <select
            ...
            <option value="" className="bg-card text-card-foreground">All Statuses</option>
            <option value="SUCCESS" className="bg-card text-card-foreground">SUCCESS</option>
            <option value="PENDING" className="bg-card text-card-foreground">PENDING</option>
            <option value="FAILED" className="bg-card text-card-foreground">FAILED</option>
          </select>
          <Button
            variant="secondary"
            size="sm"
            ...
            {loading ? (
              ...
              transactions.map(txn => (
                <TableRow key={txn.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant={txn.direction === 'RECEIVED' ? 'success' : 'warning'}>
                      {txn.direction}
                    </Badge>
                  </TableCell>
